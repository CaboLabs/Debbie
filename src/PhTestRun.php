...
class PhTestRun
{
            ...
            foreach ($reports as $test_function => $report)
            {
               ...
               if (isset($report['asserts']))
               {
                  foreach ($report['asserts'] as $assert_report)
                  {
                     ...
                     {
                        $html_report .= '<td class="text-primary">EXCEPTION: <pre>'. $assert_report['msg'] .'</pre></td>';
                     }
   ...
   public function names_tests($item, $namesSuitessubmenu)
   {
      ...
      foreach ($namesSuitessubmenu as $submenu)
      {
         ...
         {
            $Show_subitems = strlen($suites[2]) > 18 ? $Show_subitems = substr($suites[2], 0, 18) . "..." : $suites[2];
            $menu_subitems .= '<a title="' . $suites[2] . '" class="collapse-item" href="#">' . $Show_subitems . '</a>';
         }
      }
